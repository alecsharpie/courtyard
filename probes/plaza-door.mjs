...
sub('function spawnLaneAgent(room, cap){',
  'const __EC={roll:0,pullSum:0,capSum:0,plazaRoll:0,plazaIn:0,plazaRef:0,stand:0,bench:0,quayRoll:0,greenRoll:0,paraRoll:0,cycleRoll:0,tail:0};window.__EC=__EC;\nfunction spawnLaneAgent(room, cap){');
...
sub('    const bn = st ? null : freeBench();',
...
console.log(`\n-- the plaza's DOOR: 3 FOUNT_STANDS + 2 PLAZA_BENCHES, shared with every other caller --`);
...
console.log(`  of the admitted: stand ${f2(M('stand'))}  bench ${f2(M('bench'))}`);
...
if (argv.includes('--json')) console.log('JSON ' + JSON.stringify({ label: LABEL, seeds: SEEDS, days: DAYS,
  ...
  plazaRoll: +f2(M('plazaRoll')), plazaIn: +f2(M('plazaIn')), plazaRef: +f2(M('plazaRef')),
