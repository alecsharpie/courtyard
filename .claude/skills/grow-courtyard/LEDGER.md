...
**Gates** (re-run this pass against baselines pinned on HEAD): census **PASS** (`structure: unchanged`) · motion **PASS** · wide/courtyard/east/lane clean. Perf out of scope — `drawFieldEdge` runs inside `drawGround`, cache-time, no per-frame pass.
**Measured** (`probes/far-gate-waypoints.mjs`, 10 seeds × 6 days, both builds by the same code). HEAD's **170** arrivals start at **39 distinct** points scattered down the bare spine x=137.2; the tree's **173/173** enter *and* leave at `[138, 31]`, the gap, all five branches intact. Containment (`probes/field-edge-contain.mjs`, 3 seeds, day 0): far-bank strip 3.68% changed, **everything else bit-identical, peak 0**. Hedge continuous at **40/40 samples** (86-93 lum vs ~130-150 grass); the "dashes" are its scalloped crown.
**Verdict:** shipped — but built by attempt 2, which never committed. A WIP is UNPROVEN, so this pass re-measured rather than inherited; it held, and came back sharper (39 scattered starts, not 2).
**Surprise:** the world's east edge is off-canvas in **every camera but Wide** at desktop sizes — including the quarter *named* "Far bank" — and off-frame in all five on a phone. The bare arrival, and its fix, live in one of five cameras. (→ cue c182.)
**Law:** a change that shifts the `R()` stream can still be proved CONTAINED — measure at an instant BEFORE the first divergence. `farOpen()` needs `day >= 1`, so at day 0 both builds have drawn an identical stream and the diff is pure draw. That turned an alarming 4.97% day-6 diff into a known reshuffle.
**Budget:** opened **OVER** (50.5/46), seventh consecutive; `state.json`'s inventory is now the largest line item.
