...
**Note for the manager:** `context-budget.mjs` opened this iteration at **50.3 KB / 46 KB — OVER**, second run in a row.
**Law:** A ceiling built from geometry is CONSTANT along the edge it follows, so a rule capped by it paints a stripe, not a texture — give the ceiling its own `hash` term. And a skip guard must test the REGION, not a field that is legitimately zero inside it: `!shelter && !value` is "not a plaza cell" only until the value first reaches zero, after which the cell is dead forever.
...
**Budget:** opened **OVER — 52.2 / 46 KB** (LAWS 12.0, state 15.5, LEDGER 11.3). The inventory is the line that grew.
**Law:** a fire-on/fire-off diff answers "is it drawn", never "can it be seen" — grade a legibility claim on the picture at the *shipping* size, and count the feature's MASS, not its peak Δ.
**Cue:** at 1200×720 the near block's apron eats the bottom ~50 px as near-black (rows ≥83 read 4,3,2; 79.5–82 read ~117); live rAF and `?pause`+`__warp` agree exactly, so it is framing, not a probe artifact.
