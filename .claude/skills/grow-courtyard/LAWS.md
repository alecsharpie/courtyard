...
- **Locate before you judge; compare against a control you RAN — and price the brief's PREMISE the
  same way.** Check the inventory *and* grep the source, — the town usually already has the
  countdown a brief sees "no trace" of. Qualify every read by what the record IS, not what
  its name suggests. A gate that fails on HEAD is not a gate; a control returning the candidate's
  numbers is none; regenerate any fixture from `git show HEAD:` inside the probe. When a brief says a
  thing has NEVER happened, COUNT it on HEAD — 32/102 is not never. An inventory line written by the
  pass that wrote the brief is no second witness; an uncommitted WIP is UNPROVEN. (#6–#99)
- **One predicate, one definition — read a footprint back off the grid, never re-derive it.** Routes
  ...
  of which 845 are river; lane/towpath/cross are the same shape. Any count, spot-picker or CA seeded
...
- **The renderer's four traps.** (a) `project()` pinches on *screen* depth and LIFTS z NORTHWARD:
  each cell of height on a near volume walks it ~1.15 rows UP the frame, into what it should stand in
  front of — price a foreground against what is BEHIND it, and anything on it by its own row. (b) Roofs are emergent from `buildVolumes()`: change a footprint, never a roof, and two solid
  ...
  shove).
  (d) Height is read off the place under the feet (`agentZ`, `nearZ`) EVERY frame, never
  written at spawn — and a stencil averaging a 2×2 of vertices reaches a row past the cell you asked
  about. (#4–#106)
- **Time-compress everything you build.** A day is 55 s: ~2 s is state, a ~40 s trip is a resident.
  Caps set inflow, trip length sets population — floor the arrival *rate*, end a population by sending
  nobody new, and measure **presence**, not a per-instant crop. A place-holder whose visit outlasts
...
- **A walk is priced at its CHOICE and spent on its way.** Price ARRIVAL against the window's END
  ...
  hour the one branch that passes pricing is the longest walk. A window's END is the hour the
  last person is GONE, so price the walk home at the choice too, without charging one hour twice.
  ...
  prices to zero, widen the WINDOW, not the speed. Two corollaries: a quarter's
  arrival HOUR is a fact about its GATE DISTANCE, not its source's window, so count the walk from
  each gate FIRST and give it a nearer door if that is the problem; and price a choice only on what
  cannot be re-priced on ARRIVAL — a predicate that will ask again when it matters (weather, a free
  seat) must not ALSO be asked hours upstream. That is double jeopardy: it deletes the branch on
  every day the world would have allowed it. (#32–#107)
- **A feature that exists may exist at a rate of zero — count before you build on it, and advertise
  ...
  invitation joins `OFFERS`. Measure a share at the CHOICE, never by presence, which weights each
  branch by its dwell — but histogram the
  OUTCOME too, because the block your choice lands in is SHARED: timer expiry, rain, the sky and the
  failing light all reach the same "the visit is over" branch, so a guard written for one fires for
  all four and the feature runs at zero while the choice counter still says it fired. Watch the CAP's
  membership the same way: a band capped on `things.filter(p).length` counts every later population
  that also satisfies `p`, and the original starves without a line changing.
  (#7–#113)
...
- **The night's clock and the sun's hour are two axes, and the gap between them BREATHES with the
  ...
  sunset, so anything that reads as happening *at first light* must key on `sunUp`, never on the
  night's span. One level down: a predicate on the LIGHT is a predicate on the HOUR, and `!daylight`
  ...
  allow — the two pass each other without erroring. And a **hard-coded HOUR in a light term is a
  seasonal bug invisible to a screenshot**: 6.4 read as correct at the equinox, yet sat 0.6 h before
  one solstice's sunrise and 2.4 h after the other's. An hour OF THE SUN is solved at the instant it
  applies; `sunUp` read hours ahead is stale by the seasonal swing. (#12–#112)
- **The ground is a CACHE and the night is a COMPOSITE.** `drawBlocks`/`drawGround` rebuild at
  ...
  the camera ease — map it through `k = viewS/gview.s`. A night colour is set by the LAST composite
...
- **A probe's world is only as rewound as you make it, and you PIN the instant rather than wait for
  ...
  hour* — hold both fixed across any axis you vary. And a probe calling
  `lookAt(project(...))` proves the NAMING, never the POINTER: it skips the event, which is where a
  screen-space bug lives. Drive a real `mousemove`/tap at the point the thing is DISPLAYED (canvas
  coords × `rect/W`), wait past `NAME_SETTLE` 0.12 s, ONE tap per page — two taps 320 ms apart on
  mobile are a double-tap zoom, which reads exactly like a hit-test bug. The probe library is
  `.claude/skills/grow-courtyard/probes/`; the root `probes/` is a different, near-empty dir. (#3–#113)
- **A zero is evidence only if you show the test can be non-zero; a green anchor only if the
  ...
  jump test is `d > ABS_JUMP AND d > 8× that kind's MEDIAN step in 60 s`: a thing that mostly stands
  flips on the reshuffle alone, and at median **0.000** the second clause is vacuous, so any real
  motion reads as a teleport — histogram that kind's steps on HEAD instead. Its scenes are days 3, 7,
  11, 19, 22: a feature on days 12–18 is invisible. (#31–#114)
- **Judge a look from a DIFFERENCE IMAGE and a number, never two pictures in turn — and grade it at
  ...
  COHERENCE, not peak Δ, and what reads as weather is CONTRAST. A diff needs a **SAME-CODE control run**: `reseed + warp + drawScene` leaves ~1% of the
  frame unpinned at peak 90+, so quote the feature's mass as a ratio to that floor (37.4% vs 0.85%),
  never as an absolute. `filmstrip.mjs`'s Δ is a whole-frame mean at a 0.35 s gap — a RATE, blind
  under ~2% of the canvas and loud about anything global: divide by frames in the gap, split cached
  from live, crop to your feature, grade against NEIGHBOURS (`pops.mjs`). Moving a term to where
  another already peaks, price the COMPOSITE against the end of the day you did NOT touch: its own
  old value is no control, it was damped by what you just removed. To prove a gated change is
  CONTAINED, hash the canvas at pinned instants on both builds — better still, show the untouched
  AXIS is bit-identical across the year, which an `R()` reshuffle cannot fake. (#8–#114)
