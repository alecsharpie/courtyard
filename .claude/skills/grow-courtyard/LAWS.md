...
- **Price the brief's PREMISE on HEAD before you build on it.** Grep the source as well as the
  ...
  "never", and price BOTH HALVES of an array the premise names: "nothing shows X" is usually
  a claim about the branch you did not read. A claim of SCARCITY is dated the day it was made, so an inventory line written by
  the pass that wrote the brief is no witness; when you refute a premise, DELETE the sentence
  that produced it.
- **Compare against a control you RAN, and SMOKE it first.** A gate that fails on HEAD is not a
  gate, and nothing runs the shelf, so a probe breaks silently and stays broken — usually
  because the ARTIFACT crossed a limit nobody named (`courtyard.html` passed Node's 1 MiB
  `maxBuffer` at #181). A control tests a build-INDEPENDENT fact, never a predicate the
  candidate DEFINES, and differs from the thing measured in exactly ONE way.
...
- **One predicate, one definition — read a footprint back off the grid, never re-derive
  ...
  silently un-completed — so grep a widened predicate's READERS, not its callers.
...
- **A naming predicate is usually a FALL-THROUGH, so the box it answers with is not the
  surface**: intersect the GRID before counting off it, and bound what it answers for a screen
  REGION by whatever rises THROUGH it, or the tallest things in the town get called "the sky".
...
- **(c) A silhouette seen through an APERTURE takes its scale from the OPENING, not the
  body** — bound the lateral by the aperture, or a person in a 5x11 px pane comes out an egg.
...
- **A window is a SET-OUT bound or a PRESENCE bound, and the two differ by a whole visit.**
  ...
  with none on the walk fails as a population going to ZERO, never as a slower rate.
...
- **A spawn BAND is a share of a budget, not a rate**: a rare thing needs its OWN source, a
  feature that exists may exist at a rate of zero, and a hand-tuned SHARE prices the YIELD — a
  fix making each accepted coin PAY must re-price the coin.
...
- **An act whose payoff is another population's BEHAVIOUR is bounded by that population's
  ...
  away before it stopped. A ladder's lower rungs are bounded by its FIRST rung's HIT RATE, not
  by supply. Attribute an EDGE-TRIGGERED event AT THE EDGE and subtract the DELIBERATE refusals
  — a coin, a weather guard — before calling a conversion a LEAK: price the numerator's VOLUME.
...
- **The night's clock and the sun's hour are two axes, and the gap between them BREATHES
  ...
  `!daylight` / `nightF>0.3` is DAWN as well as dusk: bound a low-light rule on BOTH sides of
  noon.
...
- **The ground is a CACHE.** `drawBlocks`/`drawGround` rebuild at whatever cadence sets the
  ...
  register at cache time, repaint live, and quantize the QUANTITY, never a clock standing in
  for it. A cached layer's inputs are not its PICTURE's — what is composited over it after the blit
  (`applyLight`'s seasonal sun) is an input too, so price a "reads nothing" premise on PIXELS,
  not the function body, and what you lift back OUT carries whatever was drawn on top. Anything
  registered in SCREEN space at cache time (`FACES`) goes stale through the camera ease: map it
  through `k = viewS/gview.s`.
...
- **A probe's world is only as rewound as you make it.** `__reseed()`/`__setTime()` rewind the
  ...
  `__reseed()` REASSIGNS `R`, so a monkeypatch installed before it is eaten — instrument AFTER
  it and assert it fired. PIN `?t=` (the default entry is a DIFFERENT world) and PIN the dt
...
- **Suspect the INSTRUMENT first: a gate's PASS is evidence only about the fields it REPORTS,
  ...
  promising it, and give a seasonal system its own probe.
- **Read the REPORTER, not the producer.** `perf.mjs` is vsync-locked and blind to a pass costly
  only in a rare weather, so time the FUNCTION, in its weather, at every camera — and an
  overlay's cost is the PATH BUILD, not the fill. A THRESHOLD is priced off the subject's
  FASTEST phase, never its mean. A probe driving a real EVENT must invert the page's mapping
  term for term and assert it LANDED: a synthetic miss takes the other branch, it does not
  fail.
...
- **Judge a look from a DIFFERENCE IMAGE and a number, never two pictures in turn, and grade it
  ...
  divides by ~nothing; under mean 40 luma quote sd and RANGE.
