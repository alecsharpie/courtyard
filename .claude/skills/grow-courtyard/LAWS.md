...
`LEDGER.md`. One number per law, not three — the rest is in the archive.
Capped at 60 laws / 12 KB; bytes bind, not count.
...
- **Locate before you judge, then compare against a control you ran, not a memory.**
  ...
  itself, two loads). Same for a red gate: **a gate that fails on unmodified HEAD is
  not a gate**. (#22, #24)
...
- **A slow thing's season is not the season it is in.** Anything carrying state across
  a phase boundary — a rate-capped scalar, an *object* whose trip is an appreciable
  fraction of the cycle, or a **store that carries stock forward** — drags the previous
  quarter with it, so the phases either side of the anchor come out **unequal from code
  that is symmetric by construction**. And **two lags compose into a trough nobody
  wrote**: a market four days behind a block already behind the season made *spring*
  the thinnest quarter, not winter. Hysteresis, not an algebra bug — check the shoulder
  **pair** averages to the anchor, not each alone. (#21, #23, #29)
...
- **A ceiling is not a kill term.** If a system already ages out whatever sits at its
  ceiling, lowering the ceiling empties it by itself; a removal term on top reads as a
  cull. Give any global scalar that can approach zero per-cell variance
  (`hash(x, y+k) > c`) or it reads as a switch — but **match the grain of the variance
  to the grain the region is ADDRESSED by**: a per-CELL seventh is 2e-4 across a
  six-cell plot, so the rule was silently a no-op in the per-PLOT allotments. An
  inherited rule inherits the new region's grain, not its own meaning. (#14, #25)
...
- **A feature that exists may exist at a rate of zero — count before you build on it,
  ...
  and a new invitation reuses the existing one's rules (once ever, on the **real**
  clock, stood down on `?pause`) rather than competing with it. (#7, #9, #13; paid 3×)
...
- **The sill is DOM, and DOM fails quietly.** A CSS rule that fails to parse is
  *silent* — one comment closed a line early swallowed a whole rule and the season
  shipped as a grey chip, past a probe that checked tag, text and handler; **assert on
  computed style, not on the element**. A sill item that *borrows* space resizes the
  picture with **no `resize` event at all**, enough to put `unproject()` two cells out,
  so anything reading a screen coordinate observes the **frame**, not the window. And a
  timer a *person* races runs on the real clock: bucket it off sim `dt` and it waits
  forever on `?pause`. (#27, #28)
...
- **Pin the instant, don't wait for it.** A fixed wall-clock wait jitters by many sim
  ...
  re-derive it. Consequence: **the census ladder samples ONE warmth**, so anything
  acting only away from the anchor is invisible to it — which also makes it a clean
  attribution tool, since with the phase pinned any diff left is the reshuffle.
  (#9, #14, #16, #25)
- **`filmstrip.mjs`'s Δ is a whole-frame mean** — blind under ~2% of the canvas, loud
  ...
  first (it seeds `?t=0` then `__warp(t)`, which is not what `?t=<t>` gives you), then
  localise region-wise: a jump in every cell is weather or light, not your draw order.
  (#8, #9)
- **A CA rule that makes a region coherent makes it monotonous.** Check what
  neighbourhood inheritance does to *variety* over many cycles, not the one in front of
  you — plot-coherent re-sowing quietly lost two of the four vegetables. (#7)
