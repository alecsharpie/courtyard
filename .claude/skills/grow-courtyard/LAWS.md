...
- **Locate before you judge, then compare against a control you ran, not a memory.**
  Half of "this looks bad" is "drawn in the wrong pass". Render the same pinned instant in
  *both* builds. **A gate that fails on unmodified HEAD is not a gate**, and a control
  returning the candidate's exact numbers is not a control (`filmstrip.mjs` takes no
  `--page`). (#22, #24, #45)
...
- **Every random draw goes through `R()`** (or `hash(x,y)` per-cell). A bare
  `Math.random()` is invisible to `?seed=`.
- **Two renderer traps.** `project()` pinches on *screen* depth (revert it and a black
  seam returns beside every wall); roofs are emergent from `buildVolumes()` — change a
...
- **Any new `R()` draw reshuffles the whole seeded world** — the census churns everywhere
  ...
  not delta; run a gate that fires on what you did not build on HEAD first. **Two seeds is
  not a sample** — replay over ten. Keep each branch on the half of [0,1) it owned. (#2, #4, #5, #23, #40)
- **Time-compress everything you build.** A day is 55 s: ~2 s is state; a trip over ~40 s is a resident. Caps set inflow, trip
  length sets standing population: **a cap floor is not a population floor** — floor the
  arrival *rate*, end a population by sending nobody new. On a channel holding ONE object
  measure **presence**. **Price the walk with `pathHours()` before choosing who is sent**
  (the lane is ~25 h wide). A cap sized on daylight is CLOSED after dark: a night visitor
  needs its own presence bound. (#2, #5, #15, #23, #33, #46, #47, #51)
- **A slow world scalar wants a cap, a cycle and an anchor.** Rate-cap it, so "it never
  ...
  reduce *exactly* to the old constant at the anchor, in the **algebra** — `x * f()` is
...
- **A slow thing's season is not the season it is in.** Anything carrying state across a
  ...
  **pair** averages to the anchor. A day-scale switch on a multi-day process (a per-day wind
  on a 1.7-day boat trip) bleeds the same way: count at the choice per free sample. (#21, #23, #29, #54)
...
- **Two figures nearer than ~0.9 cells render as one shape** — hold a queue, a bench, an
  audience apart; fit a scatter by moving its centre, not clamping its members. (#4, #20)
- **A follower is a re-read RATE, not an offset.** "Target near the leader" holds a pair
  only if the follower re-targets several times faster than the leader moves and neither
...
- **A per-agent trait is a field written only at spawn.** `a.timer`, `a.greet`, `a.watch`
  count down — read one as a threshold and it cycles the band several times a second. Give
  the trait its own field (`a.wary`), predict the fraction that reacts, count it. (#6)
...
- **Stage an appearance as N clamps on one 0..1 progress**, never timed steps — run it
  backwards and it packs away for free. Keep each crossfade no wider than the threshold it
  fades in from, or the first item sits translucent at rest. (#6, #8)
- **Split a flag that gates both behaviour and drawing** — a boolean for behaviour, a 0..1
  intensity every draw site multiplies (`isWindy`/`windF`). (#15, #50)
- **`drawBlocks`/`drawGround` are a CACHED layer — and a cache's cadence is whatever sets
  its dirty flag most often** (count rebuilds with `probes/ground-rebuilds.mjs` before tuning
  a bucket). A per-frame truth on a facade or the ground needs a live overlay — register at
  cache time, repaint per frame (`drawLitPanes`); any accumulator a cache pass fills (`LIT[]`)
  is reset by that pass. `perf.mjs` saturates at 16.7 ms: time the draw call itself
  (`frame-cost.mjs`). (#39, #48, #56)
...
- **A probe's world is only as rewound as you make it.** `__reseed()` rewinds the PRNG and
  ...
  per screenshot. `__warp` sees only step boundaries and never draws. `?pause` + `__reseed`
  + `__setTime` is NOT the world a live page reaches at `?t=` (cloud 0.56 vs 0.17): to pick
  an instant for a live test, warp the live page. **`ls probes/`** before writing one: it is
  seam, not read budget. (#3, #6-#9, #29, #30, #67)
...
- **`filmstrip.mjs`'s Δ is a whole-frame mean at a 0.35 s gap — a RATE, not a step.**
  Blind under ~2% of the canvas, loud about anything global (the dusk sine reads 5–9 there,
  0.5–1.5 per frame). Divide by the frames in the gap, split cached from live
  (`probes/dusk-relight-where.mjs`), crop to your feature; grade a sample against its
  NEIGHBOURS (`pops.mjs`), never the series median. A step inside the dawn/dusk relight is
  invisible on screen — check the hour a switch fires before building a ramp for it. (#8, #9, #48, #57, #58)
...
- **An hour OF THE SUN is solved at the arrival instant, not the departure.** Anything read
  ...
  share of the seasonal swing (0.36 h/day at the equinox); fixed-point it on `sunAt(t)`. (#67)
