...
- **The ledger is not the inventory.** Check `state.json`'s inventory *and* grep the source —
  including for a TIMER: the town usually already has a countdown where a brief sees "no trace"; make it the consumer, never a second truth. (#59)
- **Locate before you judge; compare against a control you RAN — and price the brief's
  PREMISE the same way.** Half of "this looks bad" is "drawn in the wrong pass": render one pinned instant in both builds. A gate that fails on HEAD is not a gate; a control
  returning the candidate's numbers is not a control; a fixture in `/tmp` is whatever LAST
  wrote it — regenerate from `git show HEAD:` inside the probe. Twenty lines of probe first — half of b84 described a bug that did not exist. (#22, #24, #45, #86)
- **One predicate, one definition — read a footprint back off the grid, never re-derive it.**
  A second evaluation of one ellipse disagreed with `buildGrid()`'s and put 22 birds in the basin. `seed()` sows by the predicate the running rule uses. (#24, #41)
...
- **Two renderer traps.** `project()` pinches on *screen* depth (revert it and a black seam
  ...
  never a roof; two solid footprints that TOUCH are one roof (leave a gap row). (#78)
- **Any new `R()` draw reshuffles the whole seeded world** — the census churns everywhere
  ...
  delta; run a stray gate on HEAD first. **Two seeds is not a sample** — replay over ten.
...
- **Time-compress everything you build.** A day is 55 s: ~2 s is state, a ~40 s trip is a
  ...
  end a population by sending nobody new; ONE object on a channel: measure **presence** (a per-instant crop undercounts it). A
  daylight cap is CLOSED after dark. Effective lane speed is ~0.75·`a.speed`: price a trip by
  tracing one agent spawn → stopped, not `pathHours()` alone. (#2, #5, #15, #23, #33, #46, #51, #74, #85)
- **A slow world scalar wants a cap, a cycle and an anchor.** Rate-cap it, so "it never
  steps" is one number. Prefer a cosine of a phase to a ramp. Every term it replaces must
  ...
  consumer's **varying** term only; the fixed term is the floor. (#3, #12, #14, #18, #19, #22)
- **Exact at the anchor is not exact in the HISTORY.** A sign, a latch, anything that steers
  ...
  the anchor over the whole run (`signFor = () => 1`), and hash the canvas the consumer draws
  on — `gcv` is blind to live draws. A shape moved from a cache to a live
...
- **A slow thing's season is not the season it is in.** Anything carrying state across a
  ...
  **two lags compose into a trough nobody wrote**. Check the shoulder PAIR averages to the
  anchor; tune a seasonal term on a folded-time mean of its CONSUMER, never on area in its own units (a symmetric ±x% delivered +38% rain). (#21, #23, #29, #37, #54)
...
- **A ceiling is not a kill term, and a scalar is not a switch**: if a system ages out what
  ...
  makes it monotonous. Give a scalar that can reach zero per-cell variance (`hash(x, y+k) > c`) at the grain the
  region is ADDRESSED by. Split a boolean for behaviour from the 0..1
  intensity every draw site multiplies (`isWindy`/`windF`). (#7, #14, #15, #25, #50)
...
- **A feature that exists may exist at a rate of zero — count before you build on it, and
  ...
  `spawnLaneAgent` fires ~3.3×/day, so a 4% band is one person per twelve days — give a rare thing its own source. A new invitation joins `OFFERS`. (#7, #9, #13, #31)
- **Qualify every read by what the record currently IS.** `bAge` is the fallow clock only
  ...
  (`nid`) as its ONE predicate `windowLit` does, or it answers for a night that is over. "On
  the way out" is a DESTINATION, never `a.i === a.wp.length-1` — routes are extended in place
  (`goHome`, the exit push). A trait is a field written ONCE at spawn (`a.wary`) — a countdown
  read as a threshold cycles several times a second. (#6, #52, #55, #60)
- **A night arrival's budget is the places its walk REACHES, not its rate.** From an edge only 3 of 11 doors fit a 12 h night; the far side is ≥ 2.5 h from every gate. List what each entry reaches,
  price ARRIVAL against the window's END (the retire rule ends the stay, never arrival +
  dwell), and open it a trip EARLIER than the hour it is for. An event people WALK to is
  bounded at both ends by things that are not the event; its dusk-edge audience is 2–3
  people — count them before sizing anything that waits for them. (#32, #51, #53, #79)
- **`drawBlocks`/`drawGround` are a CACHED layer whose cadence is whatever sets its dirty
  ...
  what the cache drew ON TOP of it (`frame-cost.mjs`, not `perf.mjs`). The camera is `cellW/cellH/originX/topPad` with `FOCUS` fixed: anything registered in SCREEN
  space at cache time (`FACES`, `gview`) is stale for exactly the ease — map it through
  `k = viewS/gview.s` or re-register on arrival; never move `FOCUS` under a cached layer. (#39, #48, #56, #80, #81, #86)
...
- **A probe's world is only as rewound as you make it.** `__reseed()`/`__setTime()` rewind the
  PRNG and clock, not latches or agents already spawned. The renderer draws from the PRNG, so every host round-trip walks the stream: reseed, then step inside ONE
  `page.evaluate`; fresh page per screenshot. `__warp` never draws, and a canvas read after
  `requestAnimationFrame` is a frame you did not pin: call `drawScene(simT, 1/30)` yourself
  inside the evaluate. Warp the LIVE page, never `?pause`+`__setTime`; `deviceScaleFactor` 3 is a DIFFERENT world
  from 1×. `ls probes/` first. (#3, #6-#9, #29, #30, #67, #75, #83)
...
- **When a gate fails, suspect the instrument first** — three of #31's failures were the probe. A probe holding its own copy of the page's strings is a bug with a
  delay fuse: `evaluate()` can name the consts. Print a **range**, not a floored margin. Assert
  the structural, not the when. A field added to one `take()` is blind on every other `__entities` kind. The motion gate's jump is d > ABS_JUMP AND d > 8× the entity's MEDIAN step
...
- **Measure a share at the CHOICE, a threshold against the HISTOGRAM.** A share that governs
  ...
  dwell and leaks across the day roll. A threshold quoted in a brief is a hypothesis about a
  distribution: histogram the field before you tune to it. A "cut by traffic" rule is as strong as the traffic crossing the CELL — histogram by ROW. (#49, #50, #72)
- **Pin the instant, don't wait for it.** A wall-clock wait jitters by sim minutes: drive with `__warp(t)`. An instant is a *phase as well as an hour*: hold both fixed
  ...
  warmth — blind away from the anchor. A "day-of-X" sample is a sample of when X ENDS (every shower ended 21:00–02:00): pin the
  post-X state at a daylit hour. (#9, #14, #16, #25, #64)
- **`filmstrip.mjs`'s Δ is a whole-frame mean at a 0.35 s gap — a RATE, not a step.** Blind under ~2% of the canvas, loud about anything global (the dusk sine reads 5–9). Divide by frames in the gap, split cached from live, crop to your feature,
  grade against NEIGHBOURS (`pops.mjs`) — a step beside a cache-bucket rebuild hides under
  it, so print the FEATURE's own pixel series. A step inside the
  dawn/dusk relight is invisible: check the hour a switch fires before ramping it. (#8, #9, #48, #57, #58, #81)
- **A night colour is set by the LAST composite that touches it, not the pass that draws
  ...
  Check the multiply before blaming a constant. A PAGE clip needs the canvas's
  `getBoundingClientRect()` offset; a `ctx` read needs none. (#61)
- **An hour OF THE SUN is solved at the arrival instant, not the departure.** Anything read off `sunUp`/`sunDown` more than an hour or two away is stale by the day's
