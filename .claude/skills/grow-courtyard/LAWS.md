...
- **Locate before you judge; compare against a control you RAN — and price the brief's PREMISE the
  ...
  sees "no trace" of. Qualify every read by what the record IS, not what its name says. A gate
  ...
  an uncommitted WIP is UNPROVEN. A control must test a build-INDEPENDENT fact, never a
  predicate the candidate build DEFINES, or it hands back its own definition. Same trap one step
  out: an instrument built around a REGION clips the work done at its EDGE — measure a worker by
  POSITION relative to their own workplace, never by a box drawn around it. (#6–#129)
- **One predicate, one definition — read a footprint back off the grid, never re-derive it.** Routes
  ...
  FALL-THROUGH, so the box it answers with is not the surface — `pavingAt()` calls 975 cells "quay", 845 of them river. Any count, spot-picker or CA seeded off
...
- **Every random draw goes through `R()`** (or `hash(x,y)` per cell); `Math.random()` is invisible to
  ...
  calendar in every world — histogram the year. **Any new draw reshuffles the whole seeded world**: the census churns
...
- **Time is compressed, and a walk is priced at its CHOICE and spent on its way.** A day is 55 s:
  ...
  the rate: price presence as rate × visit BEFORE choosing the cap. Price ARRIVAL against the window's END (the retire rule ends the stay, not
  ...
  the walk home at the choice too, once. Walking is ~5.2 cells/h and en-route holds are unpriced:
  price N places as N × (leg + stand) before choosing N; when a window prices to zero, widen the
  WINDOW, not the speed. An arrival HOUR is a fact about
...
- **A feature that exists may exist at a rate of zero — count before you build on it.** A spawn band
  is a *share of a budget*, not a rate, so a rare thing needs its OWN source. Measure a share at the CHOICE, never by
...
- **A ceiling is not a kill term, and a scalar is not a switch**: a system that ages out what sits at
  ...
  the 0..1 intensity draw sites multiply (`isWindy`/`windF`). A skip guard tests the REGION, never a field
  legitimately zero inside it. A slow world scalar wants a rate cap and
  `tgt > x` / `tgt < x`, never `else` — AT the target the else branch runs and it flickers ±r. An accumulator-fed per-cell
...
- **The night's clock and the sun's hour are two axes, and the gap between them BREATHES with the
  ...
  SUN is solved at the instant it applies, never read hours ahead. And `day` rolls at hour 6, so it
  holds TWO dawns: index anything keyed to a morning off its own counter, never off `day`. (#12–#124)
...
- **The FRAMING decides what exists: bound the near world in DEPTH, and anchor it on the sill.**
  ...
  well the same row is 1.7 of depth lower than on the pitch. And **a quarter
...
- **A probe's world is only as rewound as you make it — and suspect the INSTRUMENT first.**
  ...
  screenshot. PIN the instant rather than wait for it — call `drawScene(simT, 1/30)`
  inside the evaluate; `__warp(t)` advances whole
  ...
  DISPLAYED (canvas coords × `rect/W`), ONE tap per page — two on mobile are a double-tap zoom,
  which reads exactly like a hit-test bug. **A gate's PASS is only evidence
  ...
  A `?pause`d page still runs rAF, so even a synchronous draw leaves the read
  unpinned: carry a SIM FINGERPRINT (clock, wind, cloud, agents) through any before/after comparison
  and refuse it unless it says NONE — one run read 18.7% changed where three repeats read 0.84%. An
  instant is VIEWPORT-dependent as well: `simT` at 1280×700 is not the weather at 1600×950. A probe
  that WRAPS a page function must forward `...args` AND the return; dropping either reports on a
...
- **Judge a look from a DIFFERENCE IMAGE and a number, never two pictures in turn — and grade it at
  ...
  COHERENCE, not peak Δ, and what reads as weather is CONTRAST. Absolute luma sd under a
  compositing wash is not a property of what you DREW — `nearShadow` scales contrast with luma, so
  sd must fall toward the viewer whatever the surface is: grade a foreground on **sd/mean**. A diff needs a **SAME-CODE control
  run**: quote the feature's mass as a ratio to that floor, never as an absolute. `filmstrip.mjs`'s Δ is a whole-frame
  RATE, blind under ~2% of the canvas and loud about anything global — crop to your feature and
  grade against NEIGHBOURS. To prove a gated change
