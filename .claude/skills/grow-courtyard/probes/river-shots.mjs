  ...
  for (const s of SHOTS) {
    ...
    const info = await page.evaluate(t => {
      ...
      return { clip: { x: r.x + a[0], y: r.y + a[1], w: b[0] - a[0], h: b[1] - a[1] },
               ...
               deep: groundCol(120, 30, grid[30 * GW + 120], hash(120, 30), maturity()) };
    }, s.t);
    await page.waitForTimeout(700);
    const path = join(OUT, `river-${s.name}-${label}.png`);
    ...
    console.log(`${label.padEnd(5)} ${s.name.padEnd(7)} hour ${info.hour}  season ${info.season}` +
      `  riverRun ${String(info.run).padEnd(5)}  cloud ${info.cloud}  boat ${info.boat ? 'yes' : 'no '}` +
