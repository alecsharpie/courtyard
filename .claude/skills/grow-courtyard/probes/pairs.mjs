...
async function run(file, seed){
  ...
  const r = await p.evaluate(() => {
    ...
    let calls = 0, roomy = 0, pairs = 0, cCalls = 0, cRoomy = 0, cPairs = 0;   // c* = the courtyard's own (#75)
    if (has){ const o = withCompanion; withCompanion = (a, room) => { const c = !a.street; if (c) cCalls++; else calls++; if (room) c ? cRoomy++ : roomy++; const n = agents.length; o(a, room); if (agents.length > n) c ? cPairs++ : pairs++; }; }
    const hist = {}, seps = []; let lanePk = 0, eastPk = 0, courtPk = 0, benchBoth = 0, benchN = 0, lost = 0, behind = 0, sitSame = 0, sitN = 0, samples = 0;
    for (let k = 0; k < 4 * 55 * 2; k++){          // two days, quarter-second steps
      ...
      lanePk = Math.max(lanePk, laneCount); eastPk = Math.max(eastPk, eastCount); courtPk = Math.max(courtPk, court);
      for (const b of agents){ if (!b.with) continue; const L = b.with;
        ...
        if (L.kind === 'sitter' && L.state === 'sit'){ benchN++; if (b.state === 'sit' && L.onBench && d >= 0.9 && d <= 1.6) benchBoth++; }
      }
    }
    seps.sort((a, b) => a - b);
    ...
    return { has, calls, roomy, pairs, cCalls, cRoomy, cPairs, benchBoth, benchN, samples, lanePk, eastPk, courtPk, lost, behind, sitSame, sitN, inBand, under, min: q(0), p10: q(0.1), p50: q(0.5), p90: q(0.9), max: q(1), hist };
  });
  await p.close();
  return { ...r, errs };
}
for (const f of FILES){
  console.log('\n== ' + f);
  const T = { calls:0, roomy:0, pairs:0, cCalls:0, cRoomy:0, cPairs:0, benchBoth:0, benchN:0, courtPk:[], samples:0, lost:0, behind:0, sitSame:0, sitN:0, inBand:0, under:0, lanePk:[], eastPk:[], hist:{} };
  for (const s of SEEDS){
    const r = await run(f, s);
    console.log(`seed ${String(s).padStart(4)}  arrivals ${r.calls} room ${r.roomy} pairs ${r.pairs}  lanePk ${r.lanePk} eastPk ${r.eastPk}  court ${r.cCalls}/${r.cRoomy}/${r.cPairs} pk ${r.courtPk} bench ${r.benchBoth}/${r.benchN}  sep p10/50/90 ${[r.p10,r.p50,r.p90].map(x=>x.toFixed(2)).join('/')} min ${r.min.toFixed(2)} max ${r.max.toFixed(2)}  behind>3 ${r.behind} lost ${r.lost} sitSame ${r.sitSame}/${r.sitN}` + (r.errs.length ? '  ERR ' + r.errs[0] : ''));
    for (const k of ['calls','roomy','pairs','cCalls','cRoomy','cPairs','benchBoth','benchN','samples','lost','behind','sitSame','sitN','inBand','under']) T[k] += r[k];
    T.lanePk.push(r.lanePk); T.eastPk.push(r.eastPk); T.courtPk.push(r.courtPk);
  ...
  console.log(`  lanePk ${T.lanePk.join(',')}  eastPk ${T.eastPk.join(',')}  courtPk ${T.courtPk.join(',')}`);
  console.log(`  COURTYARD arrivals ${T.cCalls} (room ${T.cRoomy}) pairs ${T.cPairs} = ${(100*T.cPairs/Math.max(1,T.cCalls)).toFixed(1)}% of arrivals, ${(100*T.cPairs/Math.max(1,T.cRoomy)).toFixed(1)}% of roomy; bench samples both seated in band ${T.benchBoth}/${T.benchN}`);
