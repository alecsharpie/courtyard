# The Courtyard — deep ledger archive

Entries rotated out of `LEDGER-archive.md`, which itself keeps only the last
60 `## Iteration` entries. Append-only; nothing is ever deleted. This is the
...
<!-- full text of entries 122-124, condensed in LEDGER.md by the manager pass from #124 -->
...
**Premise held; the two readings the brief asked me to reconcile were both right.** `probes/furn-band.mjs` on HEAD: 3 of 24 pieces are off-canvas in x at every desktop size (11 on a phone — #117's `ROOF_FURN[0]` at world x −7.5), and of the 21 that are on canvas, **0 are visible at 1280x700 and 1200x720**, 21 at 1600x950/1440x900, 13/13 on a phone. #117 counted "20 of 24"; c179 counted 24. The number is 21 of 21 on-canvas.
**Did.** `furnRow()` solves each piece's row from a DEPTH target instead of stating it: step north from the eaves, take the first row whose deepest DRAWN point clears the bound, reading `nearZ` at the same `floor(y)` the paint and the pointer read. Deepest point is per kind (`furnEdges`) — a box shows its base, a cord shows the hem of its longest garment 1.53 below itself. Bounded at BOTH ends, `FURN_DMAX` 81.70 / `FURN_DMIN` 79.15, with the scatter clamped to whatever each kind's own height leaves over. `ROOF_FURN` sorted by y (painter's order, now that two pieces in a bay no longer land in push order); `drawParapet` moved BEFORE the furniture and the furniture before the dormers; cords shortened to the clear run of their bay.
**Gates:** census PASS · motion PASS · filmstrip 0 POP · shots wide/courtyard/east/lane/mobile clean · perf skipped, nothing per-frame added (the solve runs once at build).
**Measured.** Drawn whole at **all five** tracked framings: 21/21 on-canvas (13/13 phone), against 0/21 at two of them on HEAD. Northmost drawn point depth **79.10** against the lane's 79.00. `probes/furn-hit.mjs`: every piece answers the pointer **for itself** at 1600x950 *and* 1280x700 — HEAD manages 21 and 0.
**Verdict:** shipped. Budget opened **OVER** (50.1 / 46).
**Surprise:** the band does not fit. Between the parapet and the shortest framing's sill there is 2.55 of depth, and a water tank is **2.549** of depth tall — it fits with six thousandths to spare and no freedom at all, which is why the scatter had to become per-kind rather than a constant. The first draft, bounded only at the sill, put twelve of the twenty-four pieces' tops over the lane's footway, where live walkers are drawn on top of them. And the census churned everywhere on a change with no `R()` in it: pinning **only** the landing board's coordinates back to HEAD made all nine cells identical, so the whole reshuffle is one cause — the perch moving with its loft changes where a bird stands, which changes whether a *later* spot is rejected for proximity.
**Law:** a solve that hands its answer to a consumer must hand over the value it TESTED — `y -= 0.01` drifts to 81.99999999, which floors to 81 in the predicate, and rounding it to 82.00 on the way out gives the draw a cell one row down the pitch. Walk the loop in whole units and return the tested value.
**Cue:** the cost is real — with everything inside the short framing's band, the roof below it is a flat empty field again at 1600x950 and on a phone, which is the defect #110 was built to fix. Nothing SHORT can be put there either: at 1280x700 the sill cuts the whole south pitch, so anything visible at all five framings must live in those 2.55 of depth.
...
**Premise held, and the hard-coded hour is worse than "a bit off".** 7.00 is *exactly* midwinter's sunrise here (`SOLAR_NOON` 12.75, `WINTER_HOURS` 11.5) — so HEAD was right in January by coincidence and three hours late in July, when the sun is up at 4.00. Not 5.2..8.1 as the brief guessed: sunrise runs **4.00 to 7.00**.
**Did.** `hearthF()` — one named definition, two consumers (the share of `CHIMNEYS` lit, each column's alpha). Morning term re-centred on `dawnEdge()`, solved at the instant it applies; half-width unchanged at `HEARTH_RUN` 3.5, so only the CENTRE moved. Height is the cold: `HEARTH_MIN` 0.16 + `HEARTH_SWING` 0.94·(1−warmth) + `HEARTH_SNOW` 0.25·snowCover. Rain and the evening term copied through untouched.
**Gates:** census PASS, **all six groups unchanged** (no new `R()`) · motion PASS · filmstrip day 0 POP · shots wide/courtyard/east/lane + jul7 & jandawn at wide and mobile · perf skipped, nothing added per frame.
**Measured** (`probes/hearth-year.mjs`, six readings, HEAD regenerated and measured by the same code): mean |peak − first light| across the year **1.26 h → 0.12 h**. At each build's own peak, midsummer lights **24/27 stacks (amt 1.00, 07:00) → 5/27 (0.16, 04:24)**; midwinter holds 21/27 and saturates at 1.09. Rain and evening bit-identical (0.700, 0.800); snow lying adds 1.095 → 1.345. In pixels, a 200 px roofline band with smoke isolated by re-rendering the same frame with `CHIMNEYS` emptied, same-code floor **0.00%**: midsummer 07:00 **1115 px → 39**, midsummer first light 119 → 0, midwinter first light 891 → 926.
**Verdict:** shipped. Budget opened **OVER at 52.3/46 KB** — the sixth of the last seven passes.
...
**Law:** two named scalars can be the same axis — check the definition before you multiply by both.
**Cue:** a stack pops on at full alpha when `hash(cx,cy+day*7)` crosses `cold*0.75`, and `day` increments at **hour 6.00**, inside the winter morning fire, reshuffling the whole lit set (10–11 stacks flip). Pre-existing and unchanged by this pass — measured on both builds — but it is the roofline's one real discontinuity.
...
**Brief:** b125 — retire the near roof's flat fill; courses at SLATE scale with staggered laps, per-slate variance, and let the surface AGE.
**Did.** `drawRoofRow` forks at `LN_WALK_S`: the town keeps its pantile quad, we get `drawSlateCell`. The lattice is GLOBAL along the row (`slateRun` walks slate indices, not cell subdivisions), so a slate straddling a cell edge is two halves of one tone with no joint — subdividing per cell would only have rebuilt the boarding at finer pitch. Courses lap DOWNSLOPE, north on rows 79-83 and south on 84-87; the gap is at the tail only, and that gap is the lap shadow. Ageing is addressed per slate: lichen (fine hash gated by a coarse batch, on a `damp` map — north pitch, the parapet's shade, the ridge's lee, the angle at an upstand), soot below the stacks on a per-stack prevailing lean, a slate replaced or slipped, and snow lodging course by course at each head. `roofMossF()`/`nearRidgeRow()` hoisted to one definition each; `tileCol`'s now-dead slate branch retired.
**Gates:** census PASS, all six groups unchanged (no new `R()`) · motion PASS · visual PASS at wide/courtyard/east/lane and 390x844 · frame-cost 3.11→3.40 ms summer, 3.01→3.58 ms winter, rebuilds identical (cache-time, as briefed).
**Numbers** (`probes/near-roof-texture.mjs`, dry midsummer noon): band sd 11.3→**20.3** at 1600x950 and 9.0→**19.5** at 390x844; changed-pixel share **74%/81%** on a same-code control floor of **0.0%**; band mean luma **fell** 98.4→93.1 and 89.6→81.5, so the foreground is still the dark thing.
**Verdict:** shipped
**Surprise:** two, and both were the instrument. (1) The APRON — the pitch running on under us off row WH — is **62 px of a 119 px band** at 1600x950, more than the nine rows of roof above it. The first build slated the roof, measured +0 below f=0.52, and had simply moved the boarding down. (2) Per-slate spread applied along the palette ramp did almost nothing: at a midsummer noon the key already pins `t` at 1, so half of every draw landed on the clamp and the spread came out one-sided. Making it a symmetric VALUE multiply took the band sd from 16.2 to 20.4 in one edit and gave back the 8 luma the joints had eaten.
**Law:** absolute luma sd under a compositing wash is not a property of what you drew. `nearShadow` reaches ~0.6 alpha by the sill and scales contrast with luma, so sd MUST fall toward the viewer whatever the surface is; grade a foreground on **sd/mean**, which here is flat at 0.18-0.23 all the way down against HEAD's collapse from 0.24 to 0.06.
**Cue:** the apron is a stylised fan, not a projection — its flare widens x by 2% while stretching y to the frame's bottom, so its courses had to be capped against the slate's width by hand rather than falling out of `project()`.
...
<!-- #126: full text, condensed in LEDGER.md to fit the per-entry cap -->
...
**Brief:** b126 — `paveWear[]` is read only by `drawPuddles`, and `pavedAt()` never reaches the roundel or the quay. Darken walked stone in the cached ground, then let those two regions pool, and make the water agree with the moss.
**Premise held and sharpened** (`probes/pave-wear.mjs`, 3 seeds × 6 days): feet cross **130/730** roundel cells and **91/130** quay cells — 765 and 390 person-seconds — and *none of it was recorded*, because the roundel is `PATH` and `pavedAt` only ever answered for `ROAD`/`SIDE` in two boxes. The line was there; nothing was reading it.
**Did.** (a) `pavedAt` widens onto `inPlaza`/`inQuay` — the **moss regions' own predicates**, so the stone that holds water is exactly the 860 cells `mossOwn[]` already carries. (b) `trodStone()` in `groundBase`, on PATH/SIDE/ROAD: `wear[]`'s idiom on the other surface, in the ground CACHE, exact at 0. `PW_FULL 0.45` is the measured p99, not a guess. (c) the decay test is now the WEAR, not the tile, plus a 6-cell sweep for quay rows 0–2 (moving the CA loop's start would have added R() draws to three rows and reshuffled the world). (d) `pudHollow[]` enters `mossTop` at `MOSS_POOL 0.45`.
**Gates:** census PASS, only `mossy 2298 → 2646` · motion PASS · filmstrip 0 POP · shots clean · perf +0.0% (vsync-capped, blind here).
**Measured.** Dry day, diff image attributed by `unproject()` vs a HEAD-vs-HEAD control: quay **51.4%** of its pixels changed (control 0%), plaza **6.7%** (control 0.22%), lane 10.4% (0%). Wet at 0.75: plaza **16.4%**, and the roundel visibly pools. Pools 560 → 792. Moss over a full year (`probes/moss-pool.mjs`): the joints that pool were **−0.009 LESS** mossy than the open stone on HEAD and are **+0.034 more** in the growing season now — a sign flip; cells that never pool moved +0.3%.
**Cost, measured because the day mean cannot see it:** `drawPuddles` 0.221 → 0.308 ms (+39%, tracking the pool count), the whole wet frame 1.83 → 1.93 ms (+5.5%), 6/6 reps. The brief's inherited "0.94 ms of a 2.0 ms frame" is 4× too big — it is 0.22 of 1.83.
**Verdict:** shipped
**Surprise:** the two halves were one line. Widening `pavedAt` bought the desire line *and* the standing water in the same edit, because the thing that was missing was never the reader — it was that `paveWear` had no way to accrue on a `PATH` cell at all. The one thing that then needed saying twice was the DECAY, which was gated on the tile and would have pinned the roundel's lines forever.
**Law:** a `?pause`d page still runs rAF, so a canvas read is unpinned even after a synchronous draw — one run of the diff gate reported 18.7% of "elsewhere" changed where three repeats since read 0.84%. Carry a SIM FINGERPRINT (clock, wind, cloud, agent-position sum) through any before/after frame comparison and refuse the number unless it says NONE.
**Cue:** `drawPuddles` still culls nothing for the camera — at a quarter camera most of the 792 pools are off-frame. Measured cost of not culling is now on record.
...
**Brief:** b127 — give mist a SECOND SOURCE over the allotments so `ghPane`'s `mistAt()` term, exactly 0 every day of every year, finally reads something.
**Did.** Found the brief's work already built and uncommitted in the tree (attempt 2, no ledger entry) — so I verified it rather than rebuilt it, per "an uncommitted WIP is UNPROVEN". `hollowMist` is a second scalar on the `cloudCover()`/`wetF()` family, RADIATIVE where the river's is evaporative: stiller, clearer, and cold-only, so a mild wet morning mists the river and not the beds. `MIST_SRC[]` makes a source a span+reach+weight and `mistAt` is the strongest at that x; the two never overlap (hollow dies at 99, river's reach starts at 106). `drawMist`'s offscreen spans only LIVE sources. The announcement stays latched on the RIVER, so a hollow-only morning is one the ticker does not name.
**Gates:** census PASS, six groups unchanged (no new `R()`) · motion PASS · visual PASS at 1600x950 and 390x844 · fogged-frame cost +3.3%, unfogged frames byte-identical.
**Measured** (`probes/hollow-year.mjs`, 3 seeds x a year): fogs **12.7%** of mornings, **0** in the 594 warm-season ones; rises to ~1, holds, gone by sunUp+4. `mistAt(88)` p50 **0.82** against HEAD's dead **0**; pane dE vs hollow-off p50 **10**, max 16. Frame A/B at a pinned instant: change confined to the allotments (maxΔ 86, 235k px >8) and **exactly 0** west of them; on a river-only morning the frame is **identical to HEAD, 0 of 6,080,000 px**.
**Verdict:** shipped
...
<!-- archived verbatim by the manager pass from #129, before condensing in place -->
...
**Brief:** b129 — give the gardener its own source so someone works the beds on most growing-season mornings.
**Premise corrected, twice.** (a) The gardener is not short of ARRIVALS: HEAD already lands 1.17 per growing day. (b) The 0.65/0.63 the brief priced off is `inWall`, and **13 of the 204 EDGE_BEDS sit OUTSIDE `wallR()`'s 27.5 square** — they are the axis beds nearest the doors, exactly the ones a near-door walk picks, so the ruler scored a working gardener zero 18% of the time. Measured by POSITION instead (at `kneelAt`), HEAD's gardener is in the beds **0.06** of a growing morning and **0.28** of a winter one: not rare, just never there while it matters. The limiter was the WALK, not the supply.
**Did.** `spawnLawnAgent(want)` takes a forced kind (fit checks unchanged — a schedule skips the lottery, not the pricing). `gardenDue()` on the bonfire's model: no roll, `hash(gardenIdx(), GARD_SALT) < GARD_K` over the beds' own season (`growF() > dieF()`), latched only when someone actually set out. `gardenIdx()` because `day` rolls at hour 6 and the window opens before it. Not under `LAWN_CAP` (`lawnCount()` drops gardeners) — the beds are a place no visitor competes for. `gardenFits()` bounds a pre-dawn walk at BOTH ends; the bed is drawn from the **near third** by walk, as #108 did for the door; `a.dawnWalk` licences the pre-dawn climb past `lawnGone()`.
**Gates:** census PASS · perf PASS +0.0% · filmstrip 0 POP · visual PASS wide/courtyard/mobile · motion FAIL `day/cart 0→1`, **dismissed on a HEAD control**: the same 2.60 step against the same 0.000 median, one scene over.
**Measured** (`probes/lawn-day.mjs`, 6 seeds × a year): in the beds on a growing morning **0.06 → 0.22 (3.7×)**; when the lawn is BUSY **0.08 → 0.26**, so it rises rather than falls. Wet 0.02 → 0.02, winter 0.28 → 0.32. **Composition: nothing was displaced** — the gardener came out of the cap, so the visitors keep all eight (at cap 32.9% → 31.1%); on a growing morning gardener +0.33, picnic +0.44, kid +0.13, sitter −0.12. Rows worked per visit 1.19 → 1.53.
**Verdict:** shipped. Budget opened **OVER at 49.0/46 KB** — five of the last six have.
**Surprise:** every lever I reached for was already in the source one level up, unapplied. #108 fixed the gardener's DOOR and left the BED a flat `pick()` over all 204; the dawn start made that worse, not better, because with the whole day ahead every bed passes `lawnFits` — the priced walk went 5.8 h → 9.2 h and the earlier start was spent entirely on crossing the town. `lawnFits` bounds the LANDING only, which is all a visitor needs because no visitor sets out before the lawn opens. Ranking the beds by walk and bounding both ends turned an 0.06 into 0.22; the schedule alone had turned it into 0.08.
**Law:** an instrument built around a region CLIPS the work done at its edge — `wallR()`'s square cuts through the very bed ring the gardener is there to work, and 18% of visits scored zero. Measure a worker by POSITION relative to their own workplace, never by a box drawn around the place they work in.
**Cue:** `probes/lawn-day.mjs` wrapped `spawnLawnAgent` as `orig()` — dropping the argument AND the return. It swallowed the forced kind and un-latched the morning, reporting 10.4 arrivals/day for a build that does not exist. Any probe that monkey-patches a spawn must forward `...args` and the return value.
## Iteration 122 — the lane stops forgetting the rain: per-cell standing water (2026-09-02) [Lane & market × New element]
